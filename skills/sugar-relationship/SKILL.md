...
description: Create a custom relationship between two SugarCRM modules (1:M or M:M) via the Extension Framework with metadata file and TableDictionary registration. Use when linking modules together (Accounts ↔ Projects, Contacts ↔ Documents, etc.).
when_to_use:
...
not_for:
  - Notes attachments (parent_id/parent_type) — use sugar-notes-attachment
  - Relate field to a single record (just a custom field) — use sugar-custom-field
related_skills:
...
Use this skill when a developer needs to link two SugarCRM modules together (custom or OOB) via a 1:M or M:M relationship. The deliverable is a metadata file under `custom/metadata/`, a TableDictionary include under `custom/Extension/application/Ext/TableDictionary/`, and (for M:M) a join table definition.

For Notes 1:M attachment (using `parent_id`/`parent_type` rather than a custom join table) see `[[sugar-notes-attachment]]`.
...
- `[[sugar-notes-attachment]]` — special-case Notes 1:M via parent_id/parent_type
- `[[sugar-package-build]]` — wrap into deployable zip
- `[[sugar-new-module]]` — when creating modules being linked together
